...
def store_question_image(image_data: bytes, user_id: str, content_type: str) -> str:
    """将题目图片存储到 MinIO，返回对象名称"""
    ext = content_type.split("/")[-1]
    if ext == "jpeg":
        ext = "jpg"
    object_name = f"questions/{user_id}/{uuid.uuid4().hex}.{ext}"
...
def crop_and_store_question_image(
    ...
    object_name = f"questions/{user_id}/cropped_{uuid.uuid4().hex}.jpg"
    storage = get_storage_service()
    storage.upload_file(cropped_bytes, object_name, "image/jpeg")
    return object_name


def get_question_image_url(object_name: str) -> Optional[str]:
    """获取题目图片的预签名 URL"""
    storage = get_storage_service()
...
def delete_question_image(object_name: str) -> bool:
    """删除 MinIO 中的题目图片"""
    storage = get_storage_service()
