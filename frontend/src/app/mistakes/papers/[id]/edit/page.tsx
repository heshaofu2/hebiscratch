...
export default function EditPaperPage() {
  ...
  // 完整题库（用于通过 ID 查找题目信息）
  const [allMistakes, setAllMistakes] = useState<MistakeListItem[]>([]);
  ...
  // 加载完整题库（不带筛选）
  const loadAllMistakes = useCallback(async () => {
    try {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      router.push(`/mistakes/papers/${id}`);
    } catch {
      // store 已处理 error
    } finally {
      setSaving(false);
  ...
  const getMistakeById = (mid: string) => allMistakes.find((m) => m._id === mid);
                  ...
                  <div className="flex-1 min-w-0">
                    {m ? (
                      <>
                        ...
                        <p className="text-gray-700 line-clamp-2 mt-1">{m.question}</p>
                      </>
                    ) : (
